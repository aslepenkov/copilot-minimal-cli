...
describe('FindCodeFilesTool', () => {
    ...
    it('should find all files in workspace', async () => {
        ...
        expect(result.codeFiles.some((file: string) => file.includes('file1.ts'))).toBe(true);
        expect(result.codeFiles.some((file: string) => file.includes('file2.js'))).toBe(true);
        expect(result.codeFiles.some((file: string) => file.includes('README.md'))).toBe(true);
        expect(result.codeFiles.some((file: string) => file.includes('package.json'))).toBe(true);
    });
    ...
    it('should exclude build directories', async () => {
        ...
        expect(result.codeFiles.some((file: string) => file.includes('file1.ts'))).toBe(true);
        expect(result.codeFiles.some((file: string) => file.includes('src/main.ts'))).toBe(true);

        // Should exclude build directories
        expect(result.codeFiles.some((file: string) => file.includes('node_modules'))).toBe(false);
        expect(result.codeFiles.some((file: string) => file.includes('dist'))).toBe(false);
        expect(result.codeFiles.some((file: string) => file.includes('build'))).toBe(false);
        expect(result.codeFiles.some((file: string) => file.includes('bin'))).toBe(false);
        expect(result.codeFiles.some((file: string) => file.includes('obj'))).toBe(false);
        expect(result.codeFiles.some((file: string) => file.includes('__pycache__'))).toBe(false);
        expect(result.codeFiles.some((file: string) => file.includes('logs'))).toBe(false);
    });
    ...
    it('should handle nested exclusions', async () => {
        ...
        expect(result.codeFiles.some((file: string) => file.includes('Button.tsx'))).toBe(true);
        expect(result.codeFiles.some((file: string) => file.includes('helpers.ts'))).toBe(true);
